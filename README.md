...
## Мысль первая
В процессе разработки можно выделить и описать набор сценариев (использования готового продукта). В сценариях можно выделить отдельный/обособленные функции/функционал и объекты, которые в них участвуют. При этом предпологаеться что один и тот-же объект может участвовать в нескольких сценариях и быть использован в нескольких функциях.
...
Тут функция - по существу тоже сценарий/алгоритм, только направленный на достижение конкретной/одной цели.
...
* объекты (структура и алгоритмы)
    ...
    * тут понятие алгоритма в более узком смысле - есть точка/состояние с которой он начинаеться, задан набор путей (последовательность действий в зависимости от исходных данных), и конечное состояние - в которое в нормальной ситуации он обязательно периведёт объект.
...
* функции/фичи - имееться в виду те вещи, которые система умеет делать, обязанности на неё возложенные
    ...
    * их воспринимаю как понятие целевой функции управления из достаточно общей теории управления (ДОТУ)
 
    * имееться в виду - есть: набор исходных ситуаций/состояний и целевое состояния. Функция для каждого из исходных/промежуточных состояний говорит что надо делать что-бы достичь целевого. Как-бы проводит из любой точки к целе.
     
    * от алгоритма отличаеться тем что несколько начальных состояний и задаёт сценарий преобразования нескольких объектов. При этом эти объекты не есть частями функции, они в ней участвуют, с ней связаны.
...
*  сценарии
    ...
    * сценарий отличаеться от алгоритма, т.к. сфокусирован на устанавлении взаимосвязи между объектами, что-бы далее, объекты функционирую по своим алгоритмам, вели систему к цели. А алгоритм сфокусирован на описании набора действий, которые надо выполнить.

## Мысль третья
Для описания каждого сценария (отдельного либо сценария функции, а так-же для описания алгоритма) удобно выделять свой язык. Т.е. для данного сценария, объекты предоставляют свои отдельные синтаксис и семантику. При этом согласования фраз разных сценариев (взаимного отображение смысла состояния объекта) ложиться на обязанности объекта (инкапсулируеться внутрь него).

## Заключение
Соответственно приведенным мыслям цель проэкта добавить возможность описания функций (с разными условиями их выполнения) и сценариев в виде Gherkin нотации (возможно несколько разширенной).
