...
Форк, которы я дорабатываю для исользование в браузере:
* Ориентирован на использование require.js
* Немного поменял синтаксис спецификаций - что-бы стандартные срдества форматирования редактора (WebStorm) выстраивали отступы правильно.
* Добавил группировку фич, назначение сценарию фонов.
* У Gherkin утверждений можно задавть приоритеты, откуда их барть: по умолчанию из описания шагов фичи, фона, группы.
* Перестроил мапинг Gherkin утверждений - каждое в своём it jasmin-а. Given/When добавлен expect(true).toBeTruthy().

Убрал расширения контекста обработчика шага, которое позволяло вызывать другие шаги в себе.
